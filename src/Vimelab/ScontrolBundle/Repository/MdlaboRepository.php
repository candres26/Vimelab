...
class MdlaboRepository extends EntityRepository
{
    ...
    public function getPage($limite = 20, $pagina = 1)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT o FROM ScontrolBundle:Mdlabo o JOIN o.mdhist h JOIN h.mdpaci p ORDER BY p.priape ASC");
    ...
    public function getFilter($parametro)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT m FROM ScontrolBundle:Mdlabo m JOIN m.mdhist l JOIN l.mdpaci k WHERE $sar ORDER BY m.id ASC");
        return $querry->getResult();
    }
}
