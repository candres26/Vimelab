...
class HslaboRepository extends EntityRepository
{
    public function getCount()
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT COUNT(o) FROM ScontrolBundle:Hslabo o");
    ...
    public function getPage($limite = 20, $pagina = 1)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT o FROM ScontrolBundle:Hslabo o JOIN o.mdpaci p ORDER BY p.priape ASC");
    ...
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT m FROM ScontrolBundle:Hslabo m JOIN m.mdpaci l WHERE l.identificacion LIKE '%$parametro%' ORDER BY m.id ASC");
        return $querry->getResult();
    }
}
