...
class MdrespRepository extends EntityRepository
{
    ...
    public function getPage($limite = 20, $pagina = 1)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT o FROM ScontrolBundle:Mdresp o");
    ...
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT m FROM ScontrolBundle:Mdresp m JOIN m.mdhist l JOIN l.mdpaci f WHERE f.identificacion LIKE '%$parametro%' or l.id LIKE '%$parametro%' ORDER BY m.id ASC");
        return $querry->getResult();
    }
}
