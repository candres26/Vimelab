...
class MdextrRepository extends EntityRepository
{
    public function getCount()
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT COUNT(o) FROM ScontrolBundle:Mdextr o");
    ...
    public function getPage($limite = 20, $pagina = 1)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT o FROM ScontrolBundle:Mdextr o JOIN o.mdhist h JOIN h.mdpaci p ORDER BY p.priape ASC");
    ...
    public function getFilter($parametro)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT m FROM ScontrolBundle:Mdextr m JOIN m.mdhist l JOIN l.mdpaci k WHERE $sar ORDER BY m.id ASC");
        return $querry->getResult();
    }
}
