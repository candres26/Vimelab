...
class TcaspeRepository extends EntityRepository
{
    public function getCount()
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT COUNT(o) FROM ScontrolBundle:Tcaspe o");
    ...
    public function getPage($limite = 20, $pagina = 1)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT o FROM ScontrolBundle:Tcaspe o ORDER BY o.nombre ASC");
    ...
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT m FROM ScontrolBundle:Tcaspe m WHERE m.nombre LIKE '%$parametro%' or m.codigo LIKE '%$parametro%' ORDER BY m.id ASC");
        return $querry->getResult();
    }
}
