...
class MdaudiRepository extends EntityRepository
{
    ...
    public function getPage($limite = 20, $pagina = 1)
    {
        ...
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT o FROM ScontrolBundle:Mdaudi o");
    ...
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT m FROM ScontrolBundle:Mdaudi m JOIN m.mdhist l JOIN l.mdpaci k WHERE l.id LIKE '%$parametro%' or k.identificacion LIKE '%$parametro%' ORDER BY m.id ASC");
        return $querry->getResult();
    }
}
