...
class GblogrRepository extends EntityRepository
{
    public function getCount()
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT COUNT(o) FROM ScontrolBundle:Gblogr o");
    ...
    {
        $em = $this->getEntityManager();
        $querry = $em->createQuery("SELECT m FROM ScontrolBundle:Gblogr m JOIN m.gbusua l WHERE m.modulo LIKE '%$parametro%' or l.nombre LIKE '%$parametro%'  or m.fecha LIKE '%$parametro%'  ORDER BY m.id ASC");
        return $querry->getResult();
    }
}
