...
class MdlaboController extends Controller
{
    ...
    public function indexAction($pag)
    {
		if(Tool::isGrant($this))
		{
			...
			$pages = $em->getRepository('ScontrolBundle:Mdlabo')->getCountPages(20);
			$pag = $pag < 1 ? 1 : $pag;
			$pag = $pag > $pages ? $pages: $pag;
			
			$entities = $em->getRepository('ScontrolBundle:Mdlabo')->getPage(20, $pag);
	...
	public function filterAction($param = '')
    {
        if(Tool::isGrant($this))
        {
            ...
            return $this->render("ScontrolBundle:Mdlabo:index.html.twig", array('entities' => $entities, 'pages' => 1, 'pag' => 1));
        }
        else
            return $this->render("ScontrolBundle::alertas.html.twig");
    }
    ...
    public function updateAction($id)
    {
		if(Tool::isGrant($this))
		{
			...
			if ($editForm->isValid()) {
				...
				Tool::logger($this, $entity->getId());
				return $this->redirect($this->generateUrl('mdlabo_show', array('id' => $id)));
			}
