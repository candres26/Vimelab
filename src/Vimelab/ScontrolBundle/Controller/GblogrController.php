...
class GblogrController extends Controller
{
    ...
    public function indexAction($pag)
    {
		if(Tool::isGrant($this))
		{
			...
			$pages = $em->getRepository('ScontrolBundle:Gblogr')->getCountPages(20);
			$pag = $pag < 1 ? 1 : $pag;
			$pag = $pag > $pages ? $pages: $pag;
			
			$entities = $em->getRepository('ScontrolBundle:Gblogr')->getPage(20, $pag);
	...
	public function filterAction($param = '')
    {
        if(Tool::isGrant($this))
        {
            ...
            return $this->render("ScontrolBundle:Gblogr:index.html.twig", array('entities' => $entities, 'pages' => 1, 'pag' => 1));
        }
        else
            return $this->render("ScontrolBundle::alertas.html.twig");
    }
